...
* **`DictManager`:**
    * `[ ] test_dict_set_get`: `d["k"] = "v"`, assert `d["k"] == "v"`.
    * `[ ] test_dict_del`: `del d["k"]`, assert `KeyError`.
    * `[ ] test_dict_ttl`: `d.set("k", "v", ttl_seconds=1)`, `sleep(1.1)`, assert `d.get("k") is None`.
    * `[ ] test_dict_len`: Assert `len(d)` is correct after adds/dels.
* **`ListManager`:**
