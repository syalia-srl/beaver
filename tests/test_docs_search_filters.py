...
def test_sync_search_with_where_and_limit():
    ...
    col.index(
        id="1",
        body={"norm_nombre": "calle 23", "tipo": "calle", "lat": 23.14, "lon": -82.38},
    )
    col.index(
        id="2",
        body={
            "norm_nombre": "hotel nacional",
            "tipo": "poi",
            "lat": 23.145,
            "lon": -82.383,
        },
    ...
    col.index(
        id="3",
        body={"norm_nombre": "calle 12", "tipo": "calle", "lat": 23.10, "lon": -82.42},
    )

    # FTS + metadata filter, synchronously
    res = col.search(
        query="calle", on=["norm_nombre"], where=[q("tipo") == "calle"], limit=5
    )
